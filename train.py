...
parser.add_argument("--ssl_weights_path", help="path to SSL weights", type=str, default="/content/drive/MyDrive/НИР/ssl_pretrained_backbone.pth")
...
# train param
parser.add_argument("--train_batch_size", help="train batch size", type=int, default=8)
...
def load_ssl_weights(backbone, ssl_path):
    """Загружает SSL веса в backbone"""
    if not os.path.exists(ssl_path):
        print(f"❌ SSL weights not found at {ssl_path}, using random initialization")
        return backbone
    
    try:
        ssl_weights = torch.load(ssl_path, map_location='cpu')
        print(f"📦 Loaded SSL weights, keys: {list(ssl_weights.keys())[:3]}...")
        
        # Получаем state_dict backbone
        backbone_dict = backbone.body.state_dict()
        loaded_count = 0
        
        # Адаптируем ключи SSL под ResNet
        for ssl_key, ssl_value in ssl_weights.items():
            # Убираем префиксы
            clean_key = ssl_key.replace('module.', '').replace('encoder.', '').replace('backbone.', '')
            
            if clean_key in backbone_dict and ssl_value.shape == backbone_dict[clean_key].shape:
                backbone_dict[clean_key] = ssl_value
                loaded_count += 1
        
        # Загружаем веса
        backbone.body.load_state_dict(backbone_dict, strict=False)
        print(f"✅ Loaded {loaded_count}/{len(backbone_dict)} SSL layers into backbone")
...
def main(args):
    ...
    # Sparse RCNN - 🔧 ИСПРАВЛЕНО ДЛЯ SSL
    backbone = resnet_fpn_backbone(args.model_name, pretrained=False)  # Всегда False для SSL
    backbone = load_ssl_weights(backbone, args.ssl_weights_path)  # Загружаем SSL веса
